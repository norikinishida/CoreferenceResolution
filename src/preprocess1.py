...
def minimize_partition(partition, args, tokenizer, pretty_tokenizer_name):
    ...
    with open(output_path, 'w') as output_file:
        for doc_key, doc_lines in documents:
            ...
            output_file.write('\n')
            doc_count += 1
    utils.writelog(f'Processed {doc_count} documents to {output_path}')
        ...
        for cluster in self.clusters.values():
            ...
            for mention in cluster:
                ...
                if existing is not None:
                    break
            if existing is not None:
                utils.writelog("Merging clusters (shouldn't happen very often)")
...
def split_into_segments(document_state: DocumentState, max_seg_len, constraints1, constraints2, tokenizer):
    ...
    while curr_idx < len(document_state.subtokens):
        ...
        while end_idx >= curr_idx and not constraints1[end_idx]:
            end_idx -= 1
        if end_idx < curr_idx:
            utils.writelog(f'{document_state.doc_key}: no sentence end found; split at token end')
